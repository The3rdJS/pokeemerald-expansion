...
static const u8 sBagMenuSortBerriesTMHMs[] =
{
    ACTION_BY_NUMBER,
    ACTION_BY_NAME,
    ACTION_DUMMY,
...
static void AddBagSortSubMenu(void)
{
    switch (gBagPosition.pocket + 1)
    {
        case POCKET_BERRIES:
        case POCKET_TM_HM:
            gBagMenu->contextMenuItemsPtr = sBagMenuSortBerriesTMHMs;
            memcpy(&gBagMenu->contextMenuItemsBuffer, &sBagMenuSortBerriesTMHMs, NELEMS(sBagMenuSortBerriesTMHMs));
            gBagMenu->contextMenuNumItems = NELEMS(sBagMenuSortBerriesTMHMs);
            break;
        default:
