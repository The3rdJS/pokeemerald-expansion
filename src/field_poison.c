...
s32 DoPoisonFieldEffect(void)
{
    ...
    {
        if (GetMonData(pokemon, MON_DATA_SANITY_HAS_SPECIES) && GetAilmentFromStatus(GetMonData(pokemon, MON_DATA_STATUS)) == AILMENT_PSN && GetMonAbility(&gParties[B_TRAINER_PLAYER][i]) != ABILITY_POISON_HEAL && GetMonAbility(&gParties[B_TRAINER_PLAYER][i]) != ABILITY_MAGIC_GUARD)
        {
            // Apply poison damage
