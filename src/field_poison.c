...
s32 DoPoisonFieldEffect(void)
{
    ...
    {
        if (GetMonData(pokemon, MON_DATA_SANITY_HAS_SPECIES) && GetAilmentFromStatus(GetMonData(pokemon, MON_DATA_STATUS)) == AILMENT_PSN && GetMonAbility(&gPlayerParty[i]) != ABILITY_POISON_HEAL && GetMonAbility(&gPlayerParty[i]) != ABILITY_MAGIC_GUARD)
        {
            // Apply poison damage
